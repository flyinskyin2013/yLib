macro(ylib_precheck_cmake_necessary_args)
    ...
    if (NOT DEFINED BUILD_YLIB_ARCH)
        ...
        if(NOT BUILD_YLIB_ARCH  MATCHES "(x86|x64|x86_64|armeabi|armeabihf|aarch64|android)")
            message(FATAL_ERROR "User must define ARG: BUILD_YLIB_ARCH. The value of it can be one of 'x86|x86_64|armeabi|armeabihf|aarch64'")
        endif()
    endif()
    ...
    if (WIN32)
        if (NOT DEFINED COMPILE_TOOL_SET)
            message(FATAL_ERROR "User must define ARG: COMPILE_TOOL_SET. The value of it can be one of 'v140,v141,v142,v143'")
        else()
            if(NOT COMPILE_TOOL_SET  MATCHES "(v140|v141|v142|v143)")
                message(FATAL_ERROR "User must define ARG: BUILD_YLIB_ARCH. The value of it can be one of 'v140,v141,v142,v143'")
            endif()
        endif()

        if (BUILD_YLIB_ARCH  MATCHES "(x64|x86_64)")
            set(COMPILE_TOOL_ARCH x64)

        elseif(BUILD_YLIB_ARCH  MATCHES "(x86)")
            set(COMPILE_TOOL_ARCH Win32)
        else()
            message(FATAL_ERROR "In window platform, BUILD_YLIB_ARCH must be 'x86|x64|x86_64'")
        endif()
        message(STATUS "COMPILE_TOOL_SET: ${COMPILE_TOOL_SET}")
        message(STATUS "COMPILE_TOOL_ARCH: ${COMPILE_TOOL_ARCH}")
    endif()
endmacro()
